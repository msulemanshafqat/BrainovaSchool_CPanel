...
public function index()
    ...
    /**
     * Loads student list for the evaluation modal.
     *
     * BUG FIX: homeworkStudent is a belongsTo() on SessionClassStudent.
     * Laravel silently ignores WHERE constraints on belongsTo eager loads —
     * with(['homeworkStudent' => fn($q) => $q->where('homework_id', x)])
     * returns the student's LAST HomeworkStudent record for ANY homework, not
     * the specific one being evaluated. This caused wrong marks to appear.
     *
     * Correct approach: load submissions separately, index by student_id,
     * then manually attach to each student row.
     */
    public function students(Request $request)
    {
        $homework = $this->repo->show($request->homework_id);

        // Load students in this class/section — no homeworkStudent eager load
        ...
        // Load submissions for THIS homework only, keyed by student_id
        $submissions = HomeworkStudent::with('homeworkUpload')
            ->where('homework_id', $homework->id)
            ->get()
            ->keyBy('student_id');

        // Attach the correct submission record to each student row
        $students->each(function ($row) use ($submissions) {
            $row->homeworkStudent = $submissions->get($row->student_id);
        });
    ...
    /**
     * View questions for a homework item.
     * Quiz: loads homework_quiz_questions (NOT online-exam question_banks).
     * Standard homework: falls back to the online-exam view questions blade.
     */
    public function viewQuestions(Request $request)
    {
        ...
        if ($homework && $homework->task_type === 'quiz') {
            $questions = DB::table('homework_quiz_questions')
                ...
                ->orderBy('id')
                ->get();
            return view('backend.homework.quiz_questions_modal',
                compact('homework', 'questions'));
        }

        $data = $homework;
        return view('backend.online-examination.online-exam.view_questions',
            compact('data'));
    }
