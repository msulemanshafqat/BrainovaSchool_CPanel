...
@section('content')
    <div class="page-content">
        ...
        <div class="col-12">
            <form action="{{ route('memory.index') }}" method="get" id="marksheed" enctype="multipart/form-data">
        ...
        <div class="table-content table-basic mt-20">
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    ...
                    @if (hasPermission('memory_create'))
                        <a href="{{ route('memory.create') }}" class="btn btn-lg ot-btn-primary">
                        ...
                        <div class="table-responsive">
                            <table class="table table-bordered role-table">
                                ...
                                <tr>
                                    ...
                                    <th class="purchase">{{ ___('settings.Memory') }}</th>
                                    <th class="purchase">{{ ___('settings.Approval Status') }}</th>
                                    <th class="purchase">{{ ___('settings.Publish Status') }}</th>
                                    <th class="purchase">{{ ___('settings.Publisher') }}</th>
                                    <th class="purchase">{{ ___('settings.Published At') }}</th>
                                ...
                                <tbody class="tbody">
                                {{-- @dd($data['students']) --}}
                                @forelse ($memories as $key => $row)
                                    <tr id="row_{{ @$row->id }}">
                                        <td class="serial">{{ ++$key }}</td>
                                        <td class="serial">{{ \Illuminate\Support\Str::limit(strip_tags($row->title), 15) }}</td>
                                        ...
                                        @if (hasPermission('memory_list') || hasPermission('memory_update'))
                                            <td class="action">
                                                <div class="dropdown dropdown-action">
                                                    ...
                                                    <ul class="dropdown-menu dropdown-menu-end ">
                                                        @if (hasPermission('memory_update'))
                                                            <li>
                                                                <a class="dropdown-item"
                                                                   href="{{ route('memory.edit', @$row->id) }}"><span
                                                                        class="icon mr-8"><i
                                                        ...
                                                        @if (hasPermission('memory_list'))
                                                            <li>
                                                                <a class="dropdown-item"
                                                                   href="{{ route('memory.show', @$row->id) }}"><span
                                                                        class="icon mr-8"><i
                                                        ...
                                                        @endif
                                                        @if (hasPermission('memory_update') && auth()->user()->role_id == \App\Enums\RoleEnum::SUPERADMIN)
                                                            @if($row->approval_status != 'approved')
                                                                <li>
                                                                    <a class="dropdown-item"
                                                                       href="{{ route('memory.changeStatus', ['id' => @$row->id,'status' => 'approved']) }}"><span
                                                                            class="icon mr-8"><i
                                                            ...
                                                            <li>
                                                                <a class="dropdown-item"
                                                                   href="{{ route('memory.changeStatus', ['id' => @$row->id,'status' => 'rejected']) }}"><span
                                                                        class="icon mr-8"><i
                                                            ...
                                                            <li>
                                                                <a class="dropdown-item"
                                                                   href="{{ route('memory.changeStatus', ['id' => @$row->id,'status' => 'pending']) }}"><span
                                                                        class="icon mr-8"><i
                                                            ...
                                                            @if (@$row->is_published)
                                                                <li>
                                                                    <a class="dropdown-item"
                                                                       href="{{ route('memory.changeStatus', ['id' => @$row->id,'status' => 'unpublished']) }}"><span
                                                                            class="icon mr-8"><i
                                                            ...
                                                            @else
                                                                <li>
                                                                    <a class="dropdown-item"
                                                                       href="{{ route('memory.changeStatus', ['id' => @$row->id,'status' => 'published']) }}"><span
                                                                            class="icon mr-8"><i
                                                        ...
                                                        @endif
                                                        @if (hasPermission('memory_delete'))
                                                            <li>
                                                                <a class="dropdown-item" href="javascript:void(0);"
                                                                   onclick="delete_row('memories/delete', {{ @$row->id }})">
                        ...
                        <div
                            ...
                            <nav aria-label="Page navigation example">
                                <ul class="pagination justify-content-between">
                                    {!! $memories->appends(\Request::capture()->except('page'))->links() !!}
                                </ul>
                            </nav>
                        </div>
                    </div>
