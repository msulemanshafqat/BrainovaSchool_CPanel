    ...
    <div class="page-content">
        ...
        <div class="card ot-card">
            <div class="card-header">
                <h4>{{ ___('settings.Parent Settings') }}</h4>
            </div>
            <div class="card-body">
                            ...
                            <div class="tab-pane fade show active" id="personal" role="tabpanel">
                                <form action="{{ route('settings.update-slider-settings') }}" enctype="multipart/form-data"
                                    ...
                                    <input type="hidden" class="row_count" name="row_count"
                                        value="{{ count($data['guardianSliders']) }}">
                                    <input type="hidden" name="user_type" value="guardian">
                                    @foreach ($data['guardianSliders'] as $key => $menu)
                                        <div class="row p-3" id="dynamicWrap">
                                            ...
                                            <div class="col-12 col-md-2 col-xl-2 col-lg-2 mb-3">
                                                <label for="is_active_{{ $loop->index }}" class="form-label">
                                                    {{ ___('settings.status') }} <span class="fillable">*</span>
                                                </label>
                                                <div class="toggle-checkbox-wrapper">
                                                    <input type="hidden" name="is_active[{{ $loop->index }}]"
                                                        value="{{ App\Enums\Status::INACTIVE }}">

                                                    <input class="toggle-checkbox" type="checkbox"
                                                        id="is_active_{{ $loop->index }}" name="slider_is_active[]"
                                                        value="{{ App\Enums\Status::ACTIVE }}"
                                                        {{ $menu->is_active == App\Enums\Status::ACTIVE ? 'checked' : '' }}>

                                                    <label class="slider-btn" for="is_active_{{ $loop->index }}"></label>
                                                </div>
                                            </div>
                                ...
                                <form action="{{ route('settings.update-menu-settings') }}" enctype="multipart/form-data"
                                    ...
                                    <input type="hidden" name="user_type" value="guardian">
                                    @foreach ($data['menus']->where('user_type', 'guardian') as $menu)
                                        <div class="row p-3">
                                            <div class="col-12 col-md-4 col-xl-4 col-lg-4 mb-3 ">
