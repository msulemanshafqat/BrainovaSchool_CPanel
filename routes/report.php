...
use App\Http\Controllers\DiscountReportController;
    ...
    Route::group(['middleware' => ['XssSanitizer']], function () {
        Route::group(['middleware' => ['lang', 'CheckSubscription', 'FeatureCheck:report']], function () {
            Route::group(['middleware' => ['auth.routes', 'AdminPanel']], function () {
                ...
                Route::controller(DiscountReportController::class)->prefix('discount-report')->group(function () {
                    Route::get('/', 'index')->name('report-discount.index')->middleware('PermissionCheck:report_discount_read');
                    Route::any('/search', 'search')->name('discount.search')->middleware('PermissionCheck:report_discount_read');
                    Route::get('/pdf-generate/{class}/{section}/{student}', 'generatePDF')->name('report-discount.pdf-generate');
                });
