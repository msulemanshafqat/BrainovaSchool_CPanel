...
@section('content')
                            ...
                            <tbody class="tbody">
                                @forelse ($data['homeworks'] as $key => $row)
                                    <tr id="row_{{ $row->id }}">
                                        ...
                                        <td>{{ $row->marks }}</td>
                                        <td><a class="btn btn-lg ot-btn-primary radius_30px small_add_btn"
                                                href="{{ @globalAsset($row->upload->path, '100X100.webp') }}"
                                                target="_blank"><i class="fa-solid fa-eye"></i></a></td>
                                        <td>{{ $row->submission_date }}
                                            @if ($row->check_submitted)
                                                <span
                                                    class="badge-basic-success-text">{{ ___('online-examination.Submitted') }}</span>
                                                <a class="btn btn-lg ot-btn-primary radius_30px small_add_btn"
                                                    href="{{ @globalAsset($row->check_submitted->homeworkUpload->path, '100X100.webp') }}"
                                                    target="_blank">
                                                    <i class="fa-solid fa-eye"></i>
                                                </a><br>
                                                <span
                                                    class="badge-basic-success-text">{{ ___('online-examination.Evaluated Marks') }}:
                                                    {{ $row->check_submitted->marks }} </span>
                                            @else
                                                <span
                                                    class="badge-basic-danger-text">{{ ___('online-examination.Not Submitted Yest') }}</span>
                                            @endif
                                        </td>
                                        
                                        <td class="action">
                                            @if(isset($row->task_type) && $row->task_type === 'quiz')
                                                
                                                {{-- IF IT IS A QUIZ --}}
                                                @if($row->check_submitted)
                                                    <span class="badge badge-success text-success">Completed</span>
                                                @else
                                                    <a href="{{ route('student-panel-homework.take-quiz', $row->id) }}" class="btn btn-sm btn-primary ot-btn-primary">
                                            ...
                                            @else
                                                ...
                                                {{-- EXISTING STANDARD HOMEWORK DROPDOWN --}}
                                                @if(!$row->check_submitted)
                                                    <div class="dropdown dropdown-action">
                                                        ...
                                                        <ul class="dropdown-menu dropdown-menu-end ">
                                                            <li>
                                                                @if (isset($row->examQuestions) && count($row->examQuestions))
                                                                    <a class="dropdown-item" target="_blank" href="{{ route('student-panel-homeworks.answer', $row->id) }}">
                                                                        <span class="icon mr-8"><i class="fa-solid fa-eye"></i></span>{{ ___('common.start') }}
                                                                    </a>
                                                                @else
                                                                    <a class="dropdown-item" href="" data-bs-toggle="modal" data-bs-target="#modalSubmitHomework" onclick="openHomeworkModal({{ $row->id }})">
                                                                        <span class="icon mr-8"><i class="fa-solid fa-save"></i></span>{{ ___('common.Submit') }}
                                                                    </a>
                                                                @endif
                                                            </li>
                                                        </ul>
    ...
    <div id="view-modal">
        <div class="modal fade" id="modalSubmitHomework" tabindex="-1" aria-labelledby="modalWidth" aria-hidden="true">
            <div class="modal-dialog modal-md">
                <div class="modal-content" id="modalWidth">
                    ...
                    <form action="{{ url('stundet/panel/homework/submit') }}" enctype="multipart/form-data" method="post"
                                    ...
                                    <div class="row">
                                        <div class="col-md-12">
                                            <label for="exampleDataList"
                                                class="form-label ">{{ ___('common.homework') }}<span class="fillable">
                                                    *</span></label>
                                            ...
                                            <div class="ot_fileUploader left-side mb-1">
                                                ...
                                                <button class="primary-btn-small-input" type="button">
                                                    ...
                                                    <input type="file" class="d-none form-control homework-file"
                                                        name="homework" id="fileBrouse"
                        ...
                        <div class="modal-footer">
                            ...
                            <button type="button" class="btn ot-btn-primary"
                                onclick="return homeworkSubmit(event)">{{ ___('ui_element.confirm') }}</button>
...
@push('script')
    ...
    <script>
        function homeworkSubmit(event) {
            ...
            // Fix the LMS developer's typo in the URL ('stundet' -> 'student')
            let actionUrl = form.attr('action').replace('stundet', 'student');
            ...
            $.ajax({
                ...
                error: function(xhr) {
                    ...
                    alert('Error: Please make sure you attached a valid document.');
