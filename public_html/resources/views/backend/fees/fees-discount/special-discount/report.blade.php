...
@section('content')
    <div class="page-content">
        ...
        <div class="page-header">
            <div class="row">
                <div class="col-sm-6">
                    <h4 class="bradecrumb-title mb-1">{{ ___('fees.Discount Report') }}</h4>
                        <ol class="breadcrumb">
        ...
        <div class="row">
            <div class="col-12">
                <form action="" method="get" id="marksheet" enctype="multipart/form-data">
                            ...
                            <div
                                ...
                                <div class="single_large_selectBox">
                                    <select class="nice-select niceSelect bordered_style wide"
                                            name="discount">
                                        <option value="">{{ ___('student_info.select discount') }} *</option>
            ...
            @if (@$data['records'] && $data['records']->count() > 0)
                <div class="col-lg-12">
                    <div class="download_print_btns mb-3">
                    ...
                    <div class="routine_wrapper" id="printableArea">
                        <div class="routine_wrapper_header">
                            <div class="routine_wrapper_header_logo">
                                <img class="header_logo" src="{{ globalAsset(setting('light_logo'), '154X38.webp') }}" alt="Logo">
                            </div>
                        </div>
                        ...
                        <div class="table-responsive">
                            <table class="table border_table mb_30">
                                ...
                                <tr>
                                    ...
                                    <th>{{ ___('fees.Type') }}</th>
                                    <th>{{ ___('fees.discount') }}</th>
                                    <th>{{ ___('fees.assigned') }}</th>
                                ...
                                @foreach ($data['records'] as $key => $record)
                                    @php
                                        ...
                                        $specialDiscount = $student->specialDiscount;
                                        $discount = $specialDiscount?->discount;
                                        $assignedBy = $specialDiscount?->assignedBy;
                                    @endphp
                                    <tr>
                                        <td>{{ $key + 1 }}</td>
                                        <td>{{ $student->first_name }} {{ $student->last_name }}</td>
                                        <td>{{ $student->admission_no }}</td>
                                        <td>{{ $record->class->name ?? '-' }} ({{ $record->section->name ?? '-' }})</td>
                                        <td>
                                          <span
                                        ...
                                        <td>{{ dateFormat($specialDiscount->assigned_date). '- '. $assignedBy->name }}</td>
                                        <td>
                                            <a href="{{route('special-fees-discount.details', $student->id)}}">
                                                <button class="btn ot-btn-primary ">{{___('common.Details')}}</button>
                                            </a>
                                        </td>
                                    </tr>
                                @endforeach
