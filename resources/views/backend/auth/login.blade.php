            ...
            <div class="card shadow-sm border-0 rounded-3">
                <div class="card-body text-center py-4">
                    <p class="mb-3 text-muted">
                        {{ ___('common.login_as_demo') }}<br>
                        <small class="text-secondary">{{___('common.demo_credentials_are_auto_filled_for_quick_access')}}</small>
                    </p>
                    <form action="{{ route('login.auth') }}" method="post" class="d-inline-block w-100"
                        ...
                        <button type="submit" class="btn btn-outline-primary w-100 py-2 fs-6 fw-semibold">
                            🚀 {{ ___('common.login_as_super_admin_demo') }}
                        </button>
                    </form>

                     <form action="{{ route('login.auth') }}" method="post" class="d-inline-block w-100 mt-3"
                        style="max-width: 300px;">
                        @csrf
                        ...
                        <input name="g-recaptcha-response" type="hidden" value="123456">
                        <button type="submit" class="btn btn-outline-primary w-100 py-2 fs-6 fw-semibold">
                            🚀 {{ ___('common.login_as_admin_demo') }}
                        </button>
                    </form>
                     ...
                     <form action="{{ route('login.auth') }}" method="post" class="d-inline-block w-100 mt-3"
                        ...
                        <input name="g-recaptcha-response" type="hidden" value="123456">
                        <button type="submit" class="btn btn-outline-primary w-100 py-2 fs-6 fw-semibold">
                            🚀 {{ ___('common.login_as_teacher_demo') }}
                        </button>
                    </form>
